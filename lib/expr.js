...
function evalExpr(expr, environment, info) {
  function _eval(expr) {
    if (expr.id) {
      var l = inferenceLabel(expr.id, environment, info);
      if (l in environment) {
    ...
    if('arg' in expr) {
      var args = environment.__arguments__;
    ...
    if('map' in expr) {
      ...
      var addr = environment.__map__;
      environment.__map__ += mapLength;
      return addr;
    }
    if('eq' in expr) {
