...
function formatArg(arg, environment) {
    ...
    if (arg.paren) {
        ...
        } else if (p.binary && p.binary.op === '+' && ['ix', 'iy'].indexOf(p.binary.args[0].id.toString().toLowerCase()) > -1) {
            var reg = p.binary.args[0].id.toString().toLowerCase();
            var offset = evalExpr({binary: {op:p.binary.op, args: _.rest(p.binary.args)}}, environment);
            if (offset < 0) {
