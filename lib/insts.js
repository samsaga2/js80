...
commands.label = function (label, js80) {
    js80.defineLabel(label, js80.image.here());
};
...
commands.asm = function (asm, js80) {
    if (asm.execmacro && asm.inst in js80.macros) {
        ...
        try {
            ...
            js80.errors.add('Invalid asm instruction', js80.env);
        }
    }
};
...
commands.include = function (include, js80) {
    ...
    if (!done) {
        js80.errors.add('File not found `' + include + "'", js80.env);
    }
};
...
commands.if = function (iff, js80) {
    ...
    if ('defined' in iff) {
        ...
        js80.errors.add('Internal error', js80.env);
        return;
    }
    if (cond) {
    ...
    _.each(commands, function (fn, key) {
        if (key in code) {
            ...
            try {
                ...
                js80.errors.add(e.message, js80.env);
            }
        }
    });
    if (!done) {
