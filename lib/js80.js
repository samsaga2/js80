...
JS80.prototype.addError = function (msg, env) {
    ...
    this.errors.add(msg, macro, lineIndex, fileName, macroLineIndex);
};
...
JS80.prototype.executeMacro = function (id, args) {
    ...
    this.environment.set('__macro__', id);
    _.each(argValues, function (finalArg, argName) {
        this.environment.set(argName, finalArg);
    }, this)
...
JS80.prototype.defineLabel = function (name, value) {
    ...
    if(currentStruct) {
        ...
        if (name[0] === '.') {
            ...
            this.environment.set('__label__', name);
        }
    ...
    var labelValue = value || this.image.here();
...
JS80.prototype.secondPassStep = function (pass) {
    try {
        ...
        if(i === 0 && !_.isNumber(addr)) {
            this.addError("Loop evaluating expr", pass.environment);
            addr = 0;
        }
    } catch (e) {
    ...
    if (!_.isNumber(addr)) {
        this.addError('Unknown label `' + addr + "'", pass.environment);
        return;
    }
    switch (pass.type) {
        case 'byte':
            if (addr < -127 || addr > 255) {
                this.addError('Value overflow', pass.environment);
            } else {
		pass.currentPage.setByte(pass.offset, addr);
            }
            break;
        case 'word':
		...
		pass.currentPage.setWord(pass.offset, addr);
            }
            break;
        case 'relative':
...
JS80.prototype.compileFile = function (fileName) {
    this.environment.push();
    this.environment.set('__filename__', fileName);
...
JS80.prototype.asm = function (code) {
    try {
        ...
        this.environment.push();
        this.environment.set('__lineIndex__', e.line);
