...
JS80.prototype.secondPassStep = function (pass) {
    try {
        for(var value = pass.expr, i=100; !_.isNumber(value), i>0; value = evalExpr(value, pass.env), i--);
        if(!_.isNumber(value)) {
            this.errors.add("Error evaluating forward expression", pass.env);
            value = 0;
        }
    } catch (e) {
    ...
    switch (pass.type) {
        case 'byte':
	    this.secondPassByteStep(pass, value);
            break;
        case 'word':
	    this.secondPassWordStep(pass, value);
            break;
        case 'relative':
	    this.secondPassRelativeStep(pass, value);
            break;
        default:
