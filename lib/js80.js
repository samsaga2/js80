...
JS80.prototype.addError = function (msg, lineIndex, fileName) {
    var error = {msg: msg};
    error.macro = this.environment.get('__macro__');
    error.fileName = fileName || this.environment.get('__filename__');
    error.lineIndex = lineIndex || this.environment.get('__lineIndex__');
    ...
    _.each(macro.args, function (macroArg) {
        if (macroArg.rest) {
            ...
            while (args.length) {
                var arg = args.shift();
                try {
                    var value = evalExpr(arg, this.environment);
                } catch (e) {
                    if (arg.id) {
                        value = arg.id;
                    } else {
...
JS80.prototype.parseInst = function (code) {
    ...
    var commands = {
        ...
        body: function (body) {
            self.parseInsts(body);
        },
        asm: function (asm) {
            if (asm.execmacro && asm.inst in self.macros) {
                self.environment.push();
                self.environment.defineLocal('__macro__', asm.inst);
        ...
        include: function (include) {
            ...
            if (!done) {
                self.addError('File not found `' + include + "'");
            }
        },
