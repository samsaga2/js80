...
JS80.prototype.defineLabel = function (name, value) {
    ...
    if(currentStruct) {
        ...
        if (name[0] === '.') {
            ...
            name = this.env.get('__label__') + name;
        } else {
            this.env.set('__label__', name);
        }
        if (this.env.get('__module__').length > 0) {
            name = this.env.get('__module__') + '.' + name;
        }
    }
    ...
    this.env.setGlobal(name, labelValue);
};
...
JS80.prototype.secondPassStep = function (pass) {
    try {
        ...
        if(i === 0 && !_.isNumber(addr)) {
            this.addError("Loop evaluating expr", pass.env);
            addr = 0;
        }
    } catch (e) {
        this.addError(e.message, pass.env);
        addr = 0;
    }
    if (!_.isNumber(addr)) {
        this.addError('Unknown label `' + addr + "'", pass.env);
        return;
    }
    switch (pass.type) {
        case 'byte':
            if (addr < -127 || addr > 255) {
                this.addError('Value overflow', pass.env);
            } else {
