...
JS80.prototype.inferenceLabel = function(label, info) {
  return expr.inferenceLabel(label, this.environment, info||this.info);
}
...
JS80.prototype.evalExpr = function(expr) {
  ...
  if('map' in expr) {
    var mapLength = this.evalExpr(expr.map);
    var addr = this.environment.__map__;
    this.environment.__map__ += mapLength;
    return addr;
  }
  if('eq' in expr) {
...
JS80.prototype.parseInst = function(code) {
  ...
  this.environment['$'] = this.image.here();
  ...
  var commands = {
    ...
    map: function(map) {
      self.environment.__map__ = self.evalExpr(map);
    },
    ds: function(ds) {
